...
def plot_aggregated_data(df, title, output_filename):
    ...
    # Create a custom color palette with a diverse set of colors
    colors = list(mcolors.TABLEAU_COLORS.values()) + list(mcolors.XKCD_COLORS.values())
    selected_colors = colors[:len(df_pivot.columns)]  # Select a subset of colors for the models

    ax = df_pivot.plot(kind='bar', figsize=(14, 8), color=selected_colors)
...
# Define paths to CSV files for each SNNI approach (Client data)
cheetah_client_path = {
...
# Define paths to CSV files for each SNNI approach (Server data)
cheetah_server_path = {
...
# Process and plot data for Cheetah server
cheetah_server_data = process_and_aggregate(cheetah_server_path)
plot_aggregated_data(cheetah_server_data, 'Cheetah Server - Data Sent by Operation Across Models', 'cheetah_server_data_sent_plot.jpeg')

# Process and plot data for SCI_HE server
sci_he_server_data = process_and_aggregate(sci_he_server_path)
plot_aggregated_data(sci_he_server_data, 'SCI_HE Server - Data Sent by Operation Across Models', 'sci_he_server_data_sent_plot.jpeg')

# Process and plot data for SCI server
sci_server_data = process_and_aggregate(sci_server_path)
plot_aggregated_data(sci_server_data, 'SCI Server - Data Sent by Operation Across Models', 'sci_server_data_sent_plot.jpeg')
