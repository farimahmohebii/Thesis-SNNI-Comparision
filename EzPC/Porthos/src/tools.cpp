...
double EstimateEnergyConsumption1(double cpuUsage, double elapsedTimeSeconds) {
...
std::chrono::high_resolution_clock::time_point startWallTime1;
...
numCores = std::thread::hardware_concurrency();
    ...
    startWallTime1 = std::chrono::high_resolution_clock::now();
    startCpuTime1 = GetCpuTime1(); 
...
void end_time()
{
  ...
  auto endWallTime1 = std::chrono::high_resolution_clock::now();
    long endCpuTime1 = GetCpuTime1();

    // Calculate elapsed times

    std::chrono::duration<double> elapsedWallTime1 = endWallTime1 - startWallTime1;
    double elapsedCpuTime1 = static_cast<double>(endCpuTime1 - startCpuTime1) / sysconf(_SC_CLK_TCK);
    ...
    double cpuUsage1 = (elapsedCpuTime1 / (elapsedWallTime1.count() * numCores)) * 100.0;

    // Output results
    std::cout << "Elapsed wall time: " << elapsedWallTime1.count() << " seconds" << std::endl;
    std::cout << "Elapsed CPU time: " << elapsedCpuTime1 << " seconds" << std::endl;
    std::cout << "CPU usage: " << cpuUsage1 << " %" << std::endl;


    double estimatedEnergy = EstimateEnergyConsumption1(cpuUsage1, elapsedTime1.count());
//    std::cout << "Elapsed time: " << elapsedTime.count() << " seconds" << std::endl;
  ...
  auto endTimer = std::chrono::high_resolution_clock::now();
	...
	cout << "------------------------------------" << endl;
	cout << "Elapsed wall time (s) =  " << diff(requestStart, requestEnd) <<endl;
	cout << "Elapsed CPU time (s) =  " << (double)(clock() - tStart)/CLOCKS_PER_SEC << endl;
#endif
	cout << "------------------------------------" << endl;	
...
*/
	cout<<"Total data sent (MiB) = "<<totalDataSent<<endl;
        cout<<"Total time in sending = "<<totalTimeInSending<<endl;
        cout<<"Total time taken (ms) = "<<totalTimeInSending+totalTimeInReceiving<<endl;
	...
	cout<<"Min size sent = "<<commObject.minSizeSent<<endl;
        cout<<"Total comm (sent+received) (MiB) = "<<totalDataSent+totalDataReceived<<endl;
...
//		<<(commObject.dataMatmul[1]/(1000.0*1000.0))<<endl;
	...
	cout<<"Total time in MatMul (s) = "<<commObject.timeMatmul[1]<<endl;
	cout<<"MatMul data sent (MiB) = "<<(commObject.dataMatmul[0]/(1000.0*1000.0))<<endl;
        cout<<"MatMul data received (MiB) = "<<(commObject.dataMatmul[1]/(1000.0*1000.0))<<endl;
        cout<<"MatMul data (sent+received) (MiB) = "<<(commObject.dataMatmul[0]/(1000.0*1000.0))+(commObject.dataMatmul[1]/(1000.0*1000.0))<<endl;
//	cout<<"relu time, data sent, data received = "
//		<<commObject.timeRelu<<" "
//		<<(commObject.dataRelu[0]/(1000.0*1000.0))<<" "
//		<<(commObject.dataRelu[1]/(1000.0*1000.0))<<endl;
	cout<<"Total time in Relu (s) = "<<commObject.timeRelu<<endl;
        cout<<"Relu data sent (MiB) = "<<(commObject.dataRelu[0]/(1000.0*1000.0))<<endl;
        cout<<"Relu data received (MiB) = "<<(commObject.dataRelu[1]/(1000.0*1000.0))<<endl;
        cout<<"Relu data (sent+received) (MiB) = "<<(commObject.dataRelu[0]/(1000.0*1000.0))+(commObject.dataRelu[1]/(1000.0*1000.0))<<endl;
...
//		<<(commObject.dataMaxPool[1]/(1000.0*1000.0))<<endl;
	...
	cout<<"Total time in MaxPool (s) = "<<commObject.timeMaxpool<<endl;
        cout<<"Maxpool data sent (MiB) = "<<(commObject.dataMaxPool[0]/(1000.0*1000.0))<<endl;
	cout<<"Maxpool data received (MiB) = "<<(commObject.dataMaxPool[1]/(1000.0*1000.0))<<endl;
        cout<<"Maxpool data (sent+received) (MiB) = "<<(commObject.dataMaxPool[0]/(1000.0*1000.0))+(commObject.dataMaxPool[1]/(1000.0*1000.0))<<endl;
	...
	cout<< "Total time in AvgPool (s) = "<<commObject.timeAvgPool<<endl;
	cout<<"Avgpool data sent (MiB) = "<<(commObject.dataAvgPool[0]/(1000.0*1000.0))<<endl;
        cout<<"Avgpool data received (MiB) = "<<(commObject.dataAvgPool[1]/(1000.0*1000.0))<<endl;
        cout<<"Avgpool data (sent+received) (MiB) = "<<(commObject.dataAvgPool[0]/(1000.0*1000.0))+(commObject.dataAvgPool[1]/(1000.0*1000.0))<<endl;
...
//		<<(commObject.dataBN[1]/(1000.0*1000.0))<<endl;
	...
	cout<<"Total time in BatchNorm (s) = "<<commObject.timeBN<<endl;
	cout<<"BatchNorm data sent (MiB) = "<<(commObject.dataBN[0]/(1000.0*1000.0))<<endl;
        cout<<"BatchNorm data received (MiB) = "<<(commObject.dataBN[1]/(1000.0*1000.0))<<endl;
        cout<<"BatchNorm data (sent+received) (MiB) = "<<(commObject.dataBN[0]/(1000.0*1000.0))+(commObject.dataBN[1]/(1000.0*1000.0))<<endl;
      //          cout<<"conv time, data sent, data received = "
    //            <<commObject.timeConv<<" "
  //              <<(commObject.dataConv[0]/(1000.0*1000.0))<<" "
//                <<(commObject.dataConv[1]/(1000.0*1000.0))<<endl;
	cout<< "Total time in Conv (s) = " << commObject.timeConv<<endl;
	cout << "Conv data sent (MiB) = "<< (commObject.dataConv[0]/(1000.0*1000.0))<<endl;
        cout << "Conv data received (MiB) = "<< (commObject.dataConv[1]/(1000.0*1000.0))<<endl;
        cout<< "Conv data (sent+received) (MiB) = "<<(commObject.dataConv[0]/(1000.0*1000.0))+(commObject.dataConv[1]/(1000.0*1000.0))<<endl;
                       // cout<<"argmax time, data sent, data received = "
               // <<commObject.timeArgmax<<" "
               // <<(commObject.dataArgmax[0]/(1000.0*1000.0))<<" "
               // <<(commObject.dataArgmax[1]/(1000.0*1000.0))<<endl;
               

        cout <<"Total time in Argmax (s) = "<<commObject.timeArgmax<<endl;
        cout <<"Argmax data sent (MiB) = "<<(commObject.dataArgmax[0]/(1000.0*1000.0))<<endl;
        cout <<"Argmax data received (MiB) = "<<(commObject.dataArgmax[1]/(1000.0*1000.0))<<endl;
        cout<<"Argmax data (sent+received) (MiB) = "<<(commObject.dataArgmax[0]/(1000.0*1000.0))+(commObject.dataArgmax[1]/(1000.0*1000.0))<<endl;

        cout <<"Total time in Truncation (s) = "<<commObject.timeTruncation<<endl;
        cout <<"Truncation data sent (MiB) = "<<(commObject.dataTruncation[0]/(1000.0*1000.0))<<endl;
        cout <<"Truncation data received (MiB) = "<<(commObject.dataTruncation[1]/(1000.0*1000.0))<<endl;
        cout << "Truncation  data (sent+received) (MiB) = "<<(commObject.dataTruncation[0]/(1000.0*1000.0))+(commObject.dataTruncation[1]/(1000.0*1000.0))<<endl;
